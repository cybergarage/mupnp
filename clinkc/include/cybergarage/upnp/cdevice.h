 ...
 *
 * \note This struct can also be cast to a CgList* and used as a node in a linked list with cg_list_* functions.
 */
...
/**
 * Check, whether the device is of specified type
 *
 * \param dev Device in question
 *
...
/**
 * Check whether the given name is either:
 * \li The unique device name (UDN)
 * \li The device's friendly name
 * \li The device's type
 *
 * \param dev Device in question
 * \param name Name to check
 * \return TRUE if the name matches with any item in the list; otherwise FALSE
 *
 */
BOOL cg_upnp_device_isname(CgUpnpDevice *dev, char *name);

/**
 * Get a child device by its name
 *
 * \param dev Device in question
 * \param name Device name to search for
 * \return NULL if not found; otherwise the child device
 *
 */
...
/**
 * Find a service from the device by the name of the service.
 *
 * \param dev Device in question
