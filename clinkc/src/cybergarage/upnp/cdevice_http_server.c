...
static void cg_upnp_device_invalidcontrolrecieved(CgSoapRequest *soapReq)
{
	...
	cg_upnp_control_soap_response_setfaultresponse(soapRes, CG_UPNP_STATUS_INVALID_ACTION, cg_upnp_status_code2string(CG_UPNP_STATUS_INVALID_ACTION));
...
#define cg_upnp_device_invalidactioncontrolrecieved(actionReq) cg_upnp_device_invalidcontrolrecieved(cg_upnp_control_action_request_getsoaprequest(actionReq))
...
#define cg_upnp_device_invalidquerycontrolrecieved(queryReq) cg_upnp_device_invalidcontrolrecieved(cg_upnp_control_query_request_getsoaprequest(queryReq))
...
static void cg_upnp_device_newsubscriptionrecieved(CgUpnpService *service, CgUpnpSubscriptionRequest *subReq)
{
	...
	callback = cg_strdup(cg_upnp_event_subscription_request_getcallback(subReq));
	if (callback == NULL)
		return;
	callback = cg_strltrim(callback, CG_UPNP_SUBSCRIPTION_CALLBACK_START_WITH, 1);
	...
	subRes = cg_upnp_event_subscription_response_new();
	cg_upnp_event_subscription_response_setstatuscode(subRes, CG_HTTP_STATUS_OK);
	cg_upnp_event_subscription_response_setsid(subRes, sid);
	cg_upnp_event_subscription_response_settimeout(subRes, timeout);
	cg_upnp_event_subscription_request_postresponse(subReq, subRes);
	cg_upnp_event_subscription_response_delete(subRes);

	cg_upnp_service_notifyallstatevariables(service);
}
