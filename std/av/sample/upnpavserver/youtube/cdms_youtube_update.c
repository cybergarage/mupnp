...
BOOL cg_upnp_dms_youtube_rssurl2flv(char* rssURL, char* flvURL, int flvURLSize)
{
  ...
  _snprintf(
#endif
      getVideoURL, sizeof(getVideoURL), "%s?%s%s&%s%s", CG_UPNP_MEDIA_YOUTUBE_GETVIDEO_URL, CG_UPNP_MEDIA_YOUTUBE_PARAM_VIDEOID, videoidParam, CG_UPNP_MEDIA_YOUTUBE_PARAM_T, tParam);
...
char* cg_upnp_dms_youtube_getupdatemessage(char* action, char* title, int cnt, int size, char* msgBuf, int msgBufSize)
{
  ...
  _snprintf(
#endif
      msgBuf, msgBufSize, "%s : %s (%d/%d)", action, title, cnt, size);
